...
function AppContent() {
  ...
  const handleMoveUp = async (taskId) => {
    ...
    try {
      const data = await api(`/api/tasks/move-task-up/${taskId}`, { method: 'POST' });
      console.log('[Frontend] handleMoveUp response:', data);
      ...
      addNotification('Move Failed', error.message || 'Could not move task up.', 'error');
    }
  };
  ...
  const handleMoveDown = async (taskId) => {
    ...
    try {
      const data = await api(`/api/tasks/move-task-down/${taskId}`, { method: 'POST' });
      console.log('[Frontend] handleMoveDown response:', data);
      setTodayTasks(data.tasks);
      ...
      addNotification('Move Failed', error.message || 'Could not move task down.', 'error');
    }
  };

  const moveBackToToday = async (taskId) => {
    try {
      const data = await api(`/api/tasks/move-back-to-today/${taskId}`, { method: 'POST' });
      ...
      setTodayTasks((prev) => {
        ...
        const filtered = prev.filter(
          (task) => task._id !== movedTask._id && task.title !== movedTask.title
        );
        // Add the moved task
      ...
      if (String(error.message).includes('409')) {
        addNotification('Duplicate Task Detected! ⚠️', 'Duplicate detected', 'warning', 6000);
      } else {
        addNotification(
          'Move Back Failed',
  ...
  const loadTasks = async () => {
    try {
      ...
      setTomorrowTasks(data.tomorrow || []);
    } catch (error) {
      console.error('Error loading tasks:', error);
      addNotification('Error Loading Tasks', 'Failed to load your tasks. Please refresh the page.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadTodaysProgress = async () => {
    try {
      const data = await api('/api/progress/today');
  ...
  const addTask = async (taskData) => {
    try {
      const data = await api('/api/tasks', { method: 'POST', body: taskData });
  ...
  const updateTask = async (taskId, updates) => {
    try {
      const data = await api(`/api/tasks/${taskId}`, { method: 'PUT', body: updates });

      // Update both today and tomorrow tasks states
      setTodayTasks((prev) => prev.map((task) => (task._id === taskId ? data : task)));
      setTomorrowTasks((prev) => prev.map((task) => (task._id === taskId ? data : task)));

      // Show completion notification
      if (updates.completed) {
        addNotification('Task Completed! ⚡', `Great job completing "${data.title}"`, 'success');
      }
  ...
  const deleteTask = async (taskId) => {
    try {
      ...
      const taskTitle = data.deletedTask?.title || 'Task';
      ...
      addNotification('Delete Failed', 'Failed to delete the task. Please try again.', 'error');
    }
  };

  const moveUncompletedTasks = async () => {
    try {
      const data = await api('/api/tasks/move-to-tomorrow', { method: 'POST' });
      ...
      if (data.movedCount === 0) {
        ...
        const newTomorrowTasks = data.tasks || [];
        ...
        addNotification('Tasks Moved Successfully! 📅', data.message, 'success', 5000);
      }
    } catch (error) {
  ...
  return (
    <div className="App">
      ...
      <main className="app-main">
        ...
        {currentView === 'history' && <CompletedTasksHistory />}
        ...
        {currentView === 'audit' && (
          <DailyAudit progressData={progressData} onAuditComplete={loadTodaysProgress} />
        )}
      </main>
    </div>
  );
